...
public class RedisLockServiceImpl implements RedisLockService{
    ...
    public boolean lock(String key, long lockMillis) {
        ...
        if (StringUtils.isEmpty(lockExpireTime) || "null".equals(lockExpireTime)) { //key被清理，锁被释放
        ...
        if (StringUtils.isEmpty(nowLowExpireTime) || "null".equals(nowLowExpireTime)) {  // key不存在，已被清理，拿锁成功
            redisTemplate.expire(key, lockMillis + 3000, TimeUnit.MILLISECONDS);
            return true;
